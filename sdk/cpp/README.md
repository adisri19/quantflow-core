...
Header-only SDK for building trading bots on quantflow platform in C or C++.

> **Note:** This is a header-only library. No package registry is used - simply copy `quantflow.h` to your project.

Uses [nlohmann/json](https://github.com/nlohmann/json) for robust JSON handling.

## Installation

1. Copy `quantflow.h` to your project
...
int main() {
    ...
    auto [bot_id, config] = quantflow::parse();
    ...
    quantflow::success("Bot executed successfully");
    return 0;
}
```
...
int main() {
    std::pair<std::string, quantflow::json> input = quantflow::parse();
    std::string bot_id = input.first;
    quantflow::json config = input.second;
...
### `quantflow::parse() -> std::pair<std::string, quantflow::json>`
...
### `quantflow::result(status, message, data)`
...
int main() {
    try {
        ...
        if (bot_id.empty()) {
            quantflow::error("Bot ID not provided");
        }

        // Validate required config
        if (!config.contains("symbol")) {
            quantflow::error("Missing required field: symbol");
        }

        std::string symbol = config["symbol"];

        // Your trading logic
        quantflow::success("Trade completed for " + symbol);
        return 0;
    } catch (const std::exception& e) {
        quantflow::error(e.what());
    }
}
```
...
std::cout << "...";  // Reserved for JSON result - use quantflow::result() instead
```
...
SDK (`quantflow.h`): Apache 2.0 - See LICENSE file in the root of this repository.
