/**
 * quantflow Query Module for C/C++
 *
 ...
 *   #include <quantflow/state.h>  // If needed in handlers
 *
 *   int main() {
 *       quantflow::query::handler("/portfolio", [](const quantflow::query::Request& req) {
 *           auto positions = quantflow::state::get_or("positions", json::array());
 *           return json{{"positions", positions}, {"count", positions.size()}};
 *       });
 *
 *       quantflow::query::handler("/status", [](const quantflow::query::Request& req) {
...
#ifndef QUANTFLOW_QUERY_H
#define QUANTFLOW_QUERY_H
 ...
 */
inline void run_server() {
    const char* port_str = std::getenv("QUANTFLOW_QUERY_PORT");
