 ...
 * {{{
 * import quantflow.Query
 * import quantflow.State  // If needed in handlers
 * }}}
 *
 * Example:
 * {{{
...
object Query {
  ...
  private def runServer(): Unit = {
    val port = sys.env.get("QUANTFLOW_QUERY_PORT").flatMap(p => Try(p.toInt).toOption).getOrElse(9476)
