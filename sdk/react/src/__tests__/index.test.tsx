...
import {
  useQuantFlowEvents,
  useBotEvents,
  QuantFlowEventsContext,
  BotEventsContext,
  BotEvent,
  BotEventUtils,
  QuantFlowEventsContextValue,
} from "../index";

describe("@quantflow/react SDK", () => {
  // Create mock utils
  ...
  const createWrapper = (contextValue: QuantFlowEventsContextValue | null) => {
    return function Wrapper({ children }: { children: React.ReactNode }) {
      return (
        <QuantFlowEventsContext.Provider value={contextValue}>
          {children}
        </QuantFlowEventsContext.Provider>
      );
    };
  };

  describe("useQuantFlowEvents", () => {
    it("should return context value when used within provider", () => {
      const mockValue = createMockContextValue();
      const wrapper = createWrapper(mockValue);

      const { result } = renderHook(() => useQuantFlowEvents(), { wrapper });
    ...
    it("should throw error when used outside provider", () => {
      ...
      if (typeof window !== "undefined") {
        delete window.__QUANTFLOW_EVENTS_CONTEXT__;
      }

      const { result } = renderHook(() => {
        try {
          return useQuantFlowEvents();
        } catch (e) {
          return e;
        }
      });
  ...
  describe("useBotEvents (alias)", () => {
    it("should be an alias for useQuantFlowEvents", () => {
      expect(useBotEvents).toBe(useQuantFlowEvents);
    });

    it("should work the same as useQuantFlowEvents", () => {
      const mockValue = createMockContextValue();
      const wrapper = createWrapper(mockValue);
  ...
  describe("Context exports", () => {
    it("should export QuantFlowEventsContext", () => {
      expect(QuantFlowEventsContext).toBeDefined();
      expect(QuantFlowEventsContext.Provider).toBeDefined();
    });

    it("should export BotEventsContext as alias", () => {
      expect(BotEventsContext).toBe(QuantFlowEventsContext);
    });
  });
  ...
  describe("Shared context via window global", () => {
    ...
    it("should return the same context instance on multiple imports", () => {
      ...
      const context1 = QuantFlowEventsContext;
