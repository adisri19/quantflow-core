...
func TestNewContainerBuilder(t *testing.T) {
	...
	assert.Contains(t, hostCfg.Tmpfs, "/var/quantflow")
}
...
func TestContainerBuilder_WithNetwork(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "custom network",
			networkName:     "quantflow-network",
			expectedNetwork: "quantflow-network",
		},
		{
...
func TestContainerBuilder_WithExecutable(t *testing.T) {
	...
	assert.Contains(t, cfg.Env, "STATE_DIR=/state/.quantflow-state")
...
func TestContainerBuilder_ChainedCalls(t *testing.T) {
	...
	// Verify host config
	assert.Equal(t, container.NetworkMode("quantflow-network"), hostCfg.NetworkMode)
