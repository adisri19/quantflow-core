...
func TestNewBotScheduleController(t *testing.T) {
	...
	assert.Equal(t, "quantflow", controller.config.Namespace)
	assert.Equal(t, 30*time.Second, controller.config.ReconcileInterval)
	assert.Equal(t, "quantflow-schedule-controller", controller.config.ControllerName)
}
...
func TestBotScheduleController_Reconcile_CreatesCronJob(t *testing.T) {
	...
	cronJob := mockCronClient.CronJobs["quantflow/schedule-schedule-1"]
...
func TestBotScheduleController_Reconcile_DeletesOrphanedCronJob(t *testing.T) {
	...
	mockCronClient := NewMockK8sCronJobClient()
	mockCronClient.CronJobs["quantflow/schedule-orphan"] = &batchv1.CronJob{
	...
	controller := NewBotScheduleController(
		BotScheduleControllerConfig{Namespace: "quantflow"},
		mockRepo,
		mockCronClient,
	)
	...
	assert.Nil(t, mockCronClient.CronJobs["quantflow/schedule-orphan"])
}
...
func TestBotScheduleController_Reconcile_UpdatesCronJobOnChange(t *testing.T) {
	...
	mockCronClient := NewMockK8sCronJobClient()
	mockCronClient.CronJobs["quantflow/schedule-schedule-1"] = &batchv1.CronJob{
		ObjectMeta: metav1.ObjectMeta{
