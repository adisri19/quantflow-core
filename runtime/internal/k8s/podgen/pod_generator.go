...
type PodGeneratorConfig struct {
	...
	// RuntimeImage is the image for init and sidecar containers (e.g., "quantflow/runtime:latest")
	RuntimeImage string
...
func NewPodGenerator(config PodGeneratorConfig) *PodGenerator {
	if config.ControllerName == "" {
		config.ControllerName = "quantflow-bot-controller"
	}
	if config.Namespace == "" {
		config.Namespace = "quantflow"
	}
