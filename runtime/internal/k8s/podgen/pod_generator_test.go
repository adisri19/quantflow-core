...
func TestPodGenerator_GeneratePod_RealtimeBot(t *testing.T) {
	...
	assert.Equal(t, "quantflow", pod.Namespace)
	...
	assert.Equal(t, "quantflow-bot-controller", pod.Labels[LabelManagedBy])
	...
	assert.Equal(t, "quantflow/runtime:latest", syncContainer.Image)
	...
	assert.Equal(t, "quantflow/runtime:latest", botContainer.Image)
	...
	assert.Equal(t, "quantflow", pod.Spec.Volumes[2].Name)
...
func TestPodGenerator_GeneratePod_WithQueryEntrypoint(t *testing.T) {
	...
	assert.Equal(t, "quantflow/runtime:latest", pod.Spec.Containers[0].Image)
	assert.Equal(t, "query-server", pod.Spec.Containers[1].Name)
	assert.Equal(t, "quantflow/runtime:latest", pod.Spec.Containers[1].Image)
...
func TestPodGenerator_GenerateScheduledPodSpec(t *testing.T) {
	...
	assert.Equal(t, "quantflow/runtime:latest", podSpec.Containers[0].Image)
