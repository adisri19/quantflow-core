...
Additional fields depend on the custom bot's schema. Use `quantflow custom-bot schema` to view required parameters.
...
quantflow bot logs <bot_id> 20250115

# Logs from a date range
quantflow bot logs <bot_id> 20250115-20250120
```
...
```bash
# Last 50 log entries
quantflow bot logs abc123 --limit 50

# Watch logs in real-time
quantflow bot logs abc123 -w

# Logs from today with limit
quantflow bot logs abc123 20250115 --limit 200
```
...
quantflow bot enable <bot_id>

# Disable a running bot
quantflow bot disable <bot_id>
```
...
```bash
quantflow bot state list <bot_id>
```
...
```bash
quantflow bot state get <bot_id> <key>
```
...
```bash
quantflow bot state delete <bot_id> <key>
```
...
```bash
quantflow bot state clear <bot_id>
```

The CLI prompts for confirmation. Use `-y` or `--yes` to skip:

```bash
quantflow bot state clear abc123 -y
```
...
```bash
quantflow bot query <bot_id> <path> [options]
```
...
```bash
# Basic query
quantflow bot query abc123 /status

# Query with parameters
quantflow bot query abc123 /signals --params '{"limit": 5}'

# Query with timeout for slow scheduled bots
quantflow bot query abc123 /portfolio --timeout 60

# Raw output for scripting
quantflow bot query abc123 /history --raw | jq '.data[0]'
```
