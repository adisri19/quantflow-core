...
The `custom-bot` commands deploy and manage custom bot definitions on quantflow platform.
...
5. Packages files into a deployable archive (respecting `.quantflowignore`)
...
```bash
quantflow custom-bot versions <name>
```

**Example:**

```bash
quantflow custom-bot versions sma-crossover
```
...
```bash
quantflow custom-bot schema <version> <name>
```
...
```bash
quantflow custom-bot schema 1.0.0 sma-crossover
```

## The .quantflowignore File

Exclude files from deployment by creating a `.quantflowignore` file. The syntax is similar to `.gitignore`:
...
Default exclusions when no `.quantflowignore` exists:
