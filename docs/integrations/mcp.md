...
description: "Enable AI assistants to interact with quantflow platform"
...
- **API Key**: Generated from your quantflow dashboard or via CLI (`quantflow auth login`)
...
The MCP server authenticates using your API key passed via the `x-api-key` header. Configure Claude Code to connect to quantflow's MCP endpoint using the CLI:

```bash
claude mcp add quantflow --transport http https://api.quantflow.io/mcp \
...
```bash
claude mcp add quantflow --transport http http://localhost:3000/mcp \
  --header "x-api-key: YOUR_API_KEY"
```
...
The output should show quantflow server as connected:

```
quantflow: https://api.quantflow.io/mcp (HTTP) - ✓ Connected
```
...
```json
{
  "mcpServers": {
    "quantflow": {
      "type": "http",
      "url": "https://api.quantflow.io/mcp",
      "headers": {
...
```bash
export QUANTFLOW_API_KEY="your-api-key-here"
claude
```

## Available Tools

quantflow's MCP server exposes 14 tools organized into six categories:
...
Once configured, interact with quantflow using natural language. The AI assistant translates your requests into the appropriate MCP tool calls.
...
```bash
claude mcp remove quantflow -s local
claude mcp remove quantflow -s project
claude mcp add quantflow --transport http https://api.quantflow.io/mcp \
  --header "x-api-key: YOUR_API_KEY"
```
...
```bash
curl -X POST https://api.quantflow.io/mcp \
...
MCP integration follows the same security model as all quantflow platform access:
