...
description: "Deploy quantflow platform for development and production"
order: 5
---

# Platform Deployment

quantflow platform consists of multiple services that work together to provide bot development, deployment, and execution capabilities. This section covers how to deploy the platform itself, whether for local development, self-hosted production, or cloud environments.

## Architecture Overview

A complete quantflow deployment includes:
...
Docker Compose provides the simplest and most tested path to running quantflow. A single `make up` command starts all services with sensible defaults. This approach works well for:
...
Public registration is disabled. Every deployment must configure `QUANTFLOW_ADMIN_EMAIL` and `QUANTFLOW_ADMIN_PASSWORD`; the API creates or syncs that root admin on startup. See [Root Admin Configuration](./admin-bootstrap).
...
3. **Configure the root admin explicitly** - Set `QUANTFLOW_ADMIN_EMAIL` and provide `QUANTFLOW_ADMIN_PASSWORD` from protected deployment configuration
