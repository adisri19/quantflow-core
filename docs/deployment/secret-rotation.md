...
```bash
quantflow local admin set --email admin@example.com
```

The command updates `~/.quantflow/compose/.env` and recreates `quantflow-api`. Existing sessions are invalidated when the configured root admin password changes.

## Kubernetes Root Admin

Root admin password should be supplied from a Secret through `quantflowApi.extraEnv`:
...
```bash
kubectl -n quantflow rollout restart deployment/quantflow-api
kubectl -n quantflow rollout status deployment/quantflow-api
```
...
```bash
kubectl -n quantflow create secret generic quantflow-root-admin \
  --from-literal=password='new-password' \
  --dry-run=client \
  -o yaml > /tmp/quantflow-root-admin.yaml

kubeseal --format yaml < /tmp/quantflow-root-admin.yaml > clusters/prod/quantflow-root-admin.sealed.yaml
```
