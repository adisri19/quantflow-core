...
description: "Start and operate a local quantflow stack with the CLI"
...
Use `quantflow local` when you want a full platform stack on one machine. The CLI writes Docker Compose files to `~/.quantflow/compose/`, manages the root admin credentials, and starts the API, frontend, docs, runtime workers, and backing services.
...
```bash
quantflow local logs          # all services
quantflow local logs api      # API service only
...
If `--password` is omitted, the CLI prompts for it. The API validates the configured password policy at startup, so check `quantflow local logs api` if the API does not become healthy after rotation.
