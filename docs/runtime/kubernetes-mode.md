...
flowchart LR
    subgraph Pod["Realtime Bot Pod"]
        ...
        Volumes["/bot, /state, /var/quantflow/logs"]
    end
...
```bash
NAMESPACE=quantflow                         # K8s namespace
MONGO_URI=mongodb://localhost:27017    # MongoDB connection
MINIO_ENDPOINT=minio:9000              # MinIO endpoint (in-cluster)
MINIO_ACCESS_KEY=quantflowadmin
MINIO_SECRET_KEY=quantflowpassword
MINIO_BUCKET=quantflow-custom-bots
```
...
./runtime controller --namespace quantflow --reconcile-interval 30s
```
