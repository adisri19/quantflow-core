---
title: "C++ Quick Start"
description: "Build your first C++ trading bot with quantflow"
...
# Fetch quantflow SDK header
FetchContent_Declare(quantflow_sdk
    GIT_REPOSITORY https://github.com/quantflow-io/quantflow.git
    GIT_TAG main
    SOURCE_SUBDIR sdk/cpp
)
FetchContent_MakeAvailable(quantflow_sdk)

# Fetch nlohmann/json (required by quantflow SDK)
...
target_include_directories(sma_bot PRIVATE ${quantflow_sdk_SOURCE_DIR})
...
std::vector<double> fetchYahooFinance(CURL* curl, const std::string& symbol) {
    ...
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "quantflow-sma-bot/1.0");
...
int main() {
    ...
    auto [botId, config] = quantflow::parse();
    ...
    quantflow::log("Bot " + botId + " started - " + symbol +
    ...
    if (!curl) {
        quantflow::error("Failed to initialize CURL");
    }
    ...
    while (true) {
        try {
            ...
            if (prices.size() < static_cast<size_t>(longPeriod)) {
                quantflow::log("Insufficient data: need " + std::to_string(longPeriod) +
            ...
            quantflow::log(std::string("Error: ") + e.what());
        }
...
The C++ SDK provides these functions in the `quantflow` namespace:

### quantflow::parse()

Reads `BOT_ID` and `BOT_CONFIG` from environment variables. Returns `std::pair<std::string, quantflow::json>`:
...
### quantflow::log(message)

Writes a log message:

```cpp
quantflow::log("Processing " + symbol);
quantflow::log("Price updated: " + std::to_string(price));
```

### quantflow::success(message)

Reports successful execution for scheduled bots:

```cpp
quantflow::success("Analysis complete");
```

### quantflow::error(message)

Reports failure and terminates with exit code 1. This function does not return:

```cpp
if (prices.empty()) {
    quantflow::error("No price data available");
}
```

### quantflow::result(data)
...
```bash
quantflow bot deploy instance-config.json
```
...
quantflow bot logs <bot_id>

# Stream logs in real-time
quantflow bot logs <bot_id> -w
