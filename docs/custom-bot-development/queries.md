...
```python [Python]
from quantflow import query, state
...
import { query, state, logger } from "@quantflow-io/quantflow-node";
...
int main() {
    quantflow::query::handler("/status", [](const quantflow::QueryRequest& req) {
        auto persisted = quantflow::state::get("bot_state");
        auto config = quantflow::query::get_config();
    ...
    quantflow::query::handler("/signals", [](const quantflow::QueryRequest& req) {
...
import qualified QuantFlow.Query as Query
import qualified QuantFlow.State as State
...
```bash
# Basic query
quantflow bot query <bot_id> /status

# With parameters
quantflow bot query <bot_id> /signals --params '{"limit": 5}'

# Raw JSON output
quantflow bot query <bot_id> /portfolio --raw
```
...
```bash
# HTTP request
curl -X POST "https://api.quantflow.io/bot/<bot_id>/query" \
