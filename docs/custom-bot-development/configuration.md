...
# Rust private crates
quantflow auth secrets set cargo-registry-token xxxxxxxxxxxxx

# C# private NuGet
quantflow auth secrets set nuget-token oy2xxxxxxxxxxxxx

# Scala private Maven
quantflow auth secrets set maven-user myuser
quantflow auth secrets set maven-token xxxxxxxxxxxxx
```

These credentials are stored securely and injected into the build environment when deploying your bot. See [Secrets](/quantflow-cli/secrets) for complete documentation.
...
The CLI also validates configuration during `quantflow custom-bot deploy` and reports detailed errors if anything is malformed.

## Related

- [Development Overview](./overview) - Bot structure and workflow
- [Bot Types](./bot-types) - Scheduled vs realtime execution
- [Secrets](/quantflow-cli/secrets) - Private dependency configuration
