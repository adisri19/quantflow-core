...
description: "Build your first Haskell trading bot with quantflow"
...
└── bin/                     # Compiled binaries (created by quantflow build)
...
source-repository-package
    type: git
    location: https://github.com/quantflow-io/quantflow.git
    tag: v1.1.0
    subdir: sdk/haskell
```
...
main = do
    ...
    case pricesResult of
        Left err -> QuantFlow.Input.error $ "Failed to fetch data: " ++ err
        Right prices ->
...
fetchYahooFinance symbol = do
    ...
    result <- try $ do
        ...
        let requestWithUA = setRequestHeader "User-Agent" ["quantflow-sma-bot/1.0"] request
...
The Haskell SDK provides these functions in the `QuantFlow.Input` module:
...
### QuantFlow.Input.error

Reports failure and terminates with exit code 1:

```haskell
if null prices
    then QuantFlow.Input.error "No price data available"
