...
description: "Build your first Scala 3 trading bot with quantflow"
...
object Main extends App {
  ...
  def fetchYahooFinance(backend: SttpBackend[Identity, Any], symbol: String): Vector[Double] = {
    ...
    val response = basicRequest
      .get(uri"$url")
      .header("User-Agent", "quantflow-sma-bot/1.0")
      .send(backend)
...
The Scala SDK provides these functions in the `quantflow.Input` object:
