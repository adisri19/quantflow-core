...
import { state } from "@quantflow-io/quantflow-node";
...
quantflow::state::set("portfolio", {
    {"AAPL", 100},
    {"GOOGL", 50}
});

// Retrieve a value (returns std::nullopt if not found)
auto portfolio = quantflow::state::get("portfolio");
if (portfolio.has_value()) {
    int aapl = (*portfolio)["AAPL"];
}

// Check if key exists
if (quantflow::state::exists("portfolio")) {
    std::cout << "Portfolio found" << std::endl;
}

// Delete a key
quantflow::state::remove("portfolio");
...
```python [Python]
from quantflow import state, metric
...
```typescript [TypeScript]
import { state, metric } from "@quantflow-io/quantflow-node";
