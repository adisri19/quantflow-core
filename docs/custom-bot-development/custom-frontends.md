...
```json
{
  ...
  "main": "index.tsx",
  "scripts": {
    "build": "quantflow-build",
    "typecheck": "tsc --noEmit"
  },
  "devDependencies": {
    "@quantflow-io/quantflow-react": "^0.2.0",
    "@types/react": "^19.0.0",
    "typescript": "^5.0.0"
  }
}
...
The `quantflow-build` script handles all bundling configuration automatically. You don't need to configure esbuild or any other build tools unless you have advanced requirements.
...
The React SDK provides the `useQuantFlowEvents` hook that connects your component to the bot's event stream. This hook returns the events array, utility functions for filtering and analysis, and loading/error states.

Here's a complete dashboard example:

```tsx
import React from "react";
import { useQuantFlowEvents, BotEvent } from "@quantflow-io/quantflow-react";

export default function Dashboard() {
  const { events, utils, loading, error } = useQuantFlowEvents();
...
The `useQuantFlowEvents` hook provides everything you need to work with your bot's event stream:
...
:root {
  --quantflow-bg: #000;
  --quantflow-card: #111;
  --quantflow-border: #333;
  --quantflow-primary: #22c55e;
  --quantflow-text: #fff;
  --quantflow-muted: #888;
}
```
...
function PortfolioChart() {
  const { utils } = useQuantFlowEvents();
  const series = utils.extractTimeSeries('portfolio_value', 'value');
...
import { reactGlobalPlugin } from "@quantflow-io/quantflow-react/esbuild";
...
```javascript
import { build } from "@quantflow-io/quantflow-react/build";
...
**Dashboard not appearing**: Ensure `frontend/package.json` exists with a `build` script that runs `quantflow-build`.

**React not found error**: Use `@quantflow-io/quantflow-react` version 0.2.0 or higher, which includes React as a peer dependency.
