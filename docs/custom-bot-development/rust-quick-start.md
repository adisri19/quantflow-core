...
description: "Build your first Rust trading bot with quantflow"
...
[dependencies]
# quantflow SDK for configuration and metrics
quantflow-sdk = "0.1"
...
The SDK is published on crates.io as `quantflow-sdk`. The binary name `sma-bot` must match the entry point path in `bot-config.yaml`.
...
fn main() {
    ...
    let client = reqwest::blocking::Client::builder()
        .user_agent("quantflow-sma-bot/1.0")
        .build()
