...
**Execution models** determine how your bot runs. quantflow supports two models: scheduled bots that run on a cron schedule (daily, hourly, every minute) and realtime bots that run continuously until stopped.

## Platform Architecture

quantflow uses a microservices architecture with the following components:

The **CLI** (`quantflow`) is the primary interface for deploying custom bots and managing bot instances. It handles dependency vendoring, compilation for compiled languages, and deployment to the platform.
...
quantflow provides official SDKs for seven languages, each offering consistent APIs for configuration parsing, result reporting, and metrics emission:

| Language | Runtime | SDK Source |
|----------|---------|------------|
| Python | python3.11 | `quantflow-sdk` (PyPI) |
| TypeScript/Node.js | nodejs20 | `@quantflow-io/quantflow-node` (GitHub Packages) |
| Rust | rust-stable | `quantflow-sdk` (crates.io) |
| C# | dotnet8 | `QuantFlow.Sdk` (NuGet) |
| Scala | scala3 | `quantflow-sdk` (GitHub Repository) |
| C++ | gcc13 | `quantflow-sdk` (GitHub Repository) |
| Haskell | ghc96 | `quantflow-sdk` (GitHub Repository) |
...
quantflow is intentionally unopinionated about trading strategies. The platform provides execution infrastructure, not trading algorithms. You bring your own strategy, data sources, and broker integrations.
...
quantflow is designed for retail algorithmic trading with execution latencies in the seconds-to-minutes range. It is not suitable for high-frequency trading (HFT) strategies that require microsecond or nanosecond execution speeds.
...
To get started with quantflow:

1. [Install the CLI](/quantflow-cli/installation) and configure authentication
