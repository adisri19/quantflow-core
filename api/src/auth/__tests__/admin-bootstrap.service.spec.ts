...
describe("AdminBootstrapService", () => {
  ...
  const originalAdminEmail = process.env.QUANTFLOW_ADMIN_EMAIL;
  const originalAdminPassword = process.env.QUANTFLOW_ADMIN_PASSWORD;
  ...
  beforeEach(() => {
    ...
    delete process.env.QUANTFLOW_ADMIN_EMAIL;
    delete process.env.QUANTFLOW_ADMIN_PASSWORD;
  ...
  afterEach(() => {
    if (originalAdminEmail === undefined) {
      ...
      process.env.QUANTFLOW_ADMIN_EMAIL = originalAdminEmail;
    }
    if (originalAdminPassword === undefined) {
      delete process.env.QUANTFLOW_ADMIN_PASSWORD;
    } else {
      process.env.QUANTFLOW_ADMIN_PASSWORD = originalAdminPassword;
    }
  ...
  it("auto-creates a configured admin on a fresh database", async () => {
    process.env.QUANTFLOW_ADMIN_EMAIL = "admin@example.com";
    process.env.QUANTFLOW_ADMIN_PASSWORD = "secret123";
  ...
  it("fails startup when admin email is missing", async () => {
    ...
    await expect(service.onModuleInit()).rejects.toThrow(
      "QUANTFLOW_ADMIN_EMAIL must be configured",
    );
  ...
  it("fails startup when admin password is missing", async () => {
    ...
    await expect(service.onModuleInit()).rejects.toThrow(
      "QUANTFLOW_ADMIN_PASSWORD must be configured",
    );
  ...
  it("fails startup when admin email is invalid", async () => {
    process.env.QUANTFLOW_ADMIN_EMAIL = "not-an-email";
    process.env.QUANTFLOW_ADMIN_PASSWORD = "secret123";
    users.count.mockResolvedValue(0);

    await expect(service.onModuleInit()).rejects.toThrow(
      "QUANTFLOW_ADMIN_EMAIL must be a valid email address",
    );
  ...
  it("fails startup when the password fails API policy", async () => {
    process.env.QUANTFLOW_ADMIN_EMAIL = "admin@example.com";
    process.env.QUANTFLOW_ADMIN_PASSWORD = "short";
    users.count.mockResolvedValue(0);

    await expect(service.onModuleInit()).rejects.toThrow(
      "QUANTFLOW_ADMIN_PASSWORD is invalid: Password must be at least 6 characters long",
    );
  ...
  it("creates the configured root admin when users exist but the configured email is absent", async () => {
    process.env.QUANTFLOW_ADMIN_EMAIL = "root@example.com";
    process.env.QUANTFLOW_ADMIN_PASSWORD = "secret123";
    users.count.mockResolvedValue(1);
    users.list.mockResolvedValue([
      user({
