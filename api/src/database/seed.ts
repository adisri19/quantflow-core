...
function getSeedAdminPassword(): string {
  const password = process.env.QUANTFLOW_SEED_ADMIN_PASSWORD;
  if (password) {
    return password;
  }
  ...
  if (!allowDefaultPassword) {
    throw new Error(
      "QUANTFLOW_SEED_ADMIN_PASSWORD is required when seeding outside development/test",
    );
  }

  logger.warn(
    "Using default development seed admin password. Set QUANTFLOW_SEED_ADMIN_PASSWORD to override.",
  );
...
async function seedDatabase() {
  ...
  try {
    ...
    logger.info(
      "Admin user created: admin@quantflow.local / configured seed password",
    );
