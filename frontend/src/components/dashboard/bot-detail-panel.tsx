...
export function BotDetailPanel({ botId }: BotDetailPanelProps) {
  ...
  const handleCopyUpdateCommand = () => {
    navigator.clipboard.writeText(`quantflow bot update ${botId} config.json`);
    toast({
    ...
    <Dialog open={isUpdateModalOpen} onOpenChange={setIsUpdateModalOpen}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Use the quantflow CLI to update your bot configuration.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input
                value={`quantflow bot update ${botId} config.json`}
                readOnly
                className="font-mono text-sm bg-muted"
              />
              <Button
            ...
            <p className="text-xs text-muted-foreground">
              Run this command in your terminal where the quantflow CLI is installed.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2 text-xs font-mono bg-muted p-4 rounded-lg">
              ...
              <p>quantflow bot list</p>
              <p className="text-muted-foreground mt-2"># View bot logs</p>
              <p>quantflow bot logs {botId}</p>
              <p className="text-muted-foreground mt-2"># Delete bot</p>
              <p>quantflow bot delete {botId}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <ExternalLink className="h-4 w-4 text-muted-foreground" />
            <a
              href="https://docs.quantflow.io/quantflow-CLI/bot-commands"
              target="_blank"
