#!/bin/sh
# quantflow CLI installer
# Usage: curl -sSL https://install.quantflow.io | sh
set -eu

GITHUB_REPO="quantflow-io/quantflow"
INSTALL_DIR="$HOME/.quantflow/bin"
BINARY_NAME="quantflow"
WORK_DIR=""
...
setup_path() {
    ...
    EXPORT_LINE="export PATH=\"\$HOME/.quantflow/bin:\$PATH\""
    MODIFIED_FILES=""
    ALREADY_CONFIGURED=""
    ...
    append_to_file() {
        ...
        if [ -f "$file" ]; then
            ...
            if ! grep -qF '.quantflow/bin' "$file" 2>/dev/null; then
                printf '\n# quantflow CLI\n%s\n' "$EXPORT_LINE" >> "$file"
    ...
    if [ -z "$MODIFIED_FILES" ] && [ -z "$ALREADY_CONFIGURED" ]; then
        if [ -f "$HOME/.profile" ]; then
            if ! grep -qF '.quantflow/bin' "$HOME/.profile" 2>/dev/null; then
    ...
    if [ -n "$MODIFIED_FILES" ]; then
        ...
        printf "  Run %ssource <file>%s or open a new terminal to use quantflow.\n" "$BOLD" "$RESET"
    else
...
main() {
    ...
    printf "\n"
    success "quantflow ${TAG} installed successfully!"
    printf "  Run %squantflow --help%s to get started.\n\n" "$BOLD" "$RESET"
}
