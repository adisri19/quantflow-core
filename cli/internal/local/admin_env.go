...
const (
	adminEmailEnvKey    = "QUANTFLOW_ADMIN_EMAIL"
	adminPasswordEnvKey = "QUANTFLOW_ADMIN_PASSWORD"
)

// SetAdminEmail updates QUANTFLOW_ADMIN_EMAIL in the local compose .env file.
func SetAdminEmail(composeDir string, email string) error {
	return SetAdminCredentials(composeDir, email, "")
}

// SetAdminPassword updates QUANTFLOW_ADMIN_PASSWORD while preserving the configured
// admin email.
func SetAdminPassword(composeDir string, password string) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return fmt.Errorf(".env not found in %s; run `quantflow local init` first", composeDir)
		}
	...
	if envValue(content, adminEmailEnvKey) == "" {
		return fmt.Errorf("QUANTFLOW_ADMIN_EMAIL is not configured; run `quantflow local admin set --email you@example.com` first")
	}
...
// SetAdminCredentials updates QUANTFLOW_ADMIN_EMAIL and QUANTFLOW_ADMIN_PASSWORD in the
...
func AdminCredentialsConfigured(composeDir string) (bool, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return false, fmt.Errorf(".env not found in %s; run `quantflow local init` first", composeDir)
		}
