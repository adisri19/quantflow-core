...
// ValidateRepoPath checks that the given path looks like a quantflow repo
func ValidateRepoPath(path string) error {
	...
	if len(missing) > 0 {
		return fmt.Errorf("not a valid quantflow repository (missing: %s): %s", strings.Join(missing, ", "), absPath)
	}
