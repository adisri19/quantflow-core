...
func LoadAuth() (*Auth, error) {
	...
	if env == nil || env.APIKey == "" {
		return nil, fmt.Errorf("no API key configured (run `quantflow auth login` or `quantflow env add`)")
	}
...
// ~/.quantflow/auth.json so brand-new users don't need to set up environments
...
func PromptForNewAPIKey() (*Auth, error) {
	...
	logger.Printf("Enter your quantflow API key: ")
