...
// IgnoreParser handles parsing and matching of .quantflowignore files
...
// LoadIgnoreFile loads patterns from a .quantflowignore file
...
func (p *IgnoreParser) AddDefaultPatterns() {
	...
	// Projects can add them to .quantflowignore if they want to exclude them.
	defaults := []string{
		"*.log", "*.tmp", "*.temp",
...
func (p *IgnoreParser) IsIgnored(relPath string, isDir bool) bool {
	...
	// often contain large build artifacts. Users should use .quantflowignore to exclude
...
func CreateIgnoreParserForDir(dir string) (*IgnoreParser, error) {
	...
	// Load .quantflowignore file if it exists
	ignoreFilePath := filepath.Join(dir, ".quantflowignore")
