...
func withTempConfigDir(t *testing.T) string {
	...
	prevEnv := os.Getenv("QUANTFLOW_API_URL")
	SetConfigDir(dir)
	SetActiveEnvOverride("")
	os.Unsetenv("QUANTFLOW_API_URL")
	t.Cleanup(func() {
		SetConfigDir(prevCfg)
		SetActiveEnvOverride(prevOverride)
		if prevEnv != "" {
			os.Setenv("QUANTFLOW_API_URL", prevEnv)
		} else {
			os.Unsetenv("QUANTFLOW_API_URL")
		}
	})
...
func TestSaveAndLoadEnvironments_RoundTrip(t *testing.T) {
	...
	original := &Environments{
		...
		Environments: map[string]Environment{
			"local": {URL: "http://localhost:3000", APIKey: "quantflow_local", CreatedAt: time.Now().UTC().Truncate(time.Second)},
			"prod":  {URL: "https://api.quantflow.io", APIKey: "quantflow_prod", CreatedAt: time.Now().UTC().Truncate(time.Second)},
		},
	}
	...
	if loaded.Environments["prod"].URL != "https://api.quantflow.io" {
		t.Errorf("prod URL mismatch: got %q", loaded.Environments["prod"].URL)
	}
	if loaded.Environments["prod"].APIKey != "quantflow_prod" {
...
func TestEnvironments_Use(t *testing.T) {
	envs := &Environments{Environments: map[string]Environment{
		"local": {URL: "http://localhost:3000"},
		"prod":  {URL: "https://api.quantflow.io"},
	}}
...
func TestResolveActive_OverrideFlagWins(t *testing.T) {
	...
	envs := &Environments{
		...
		Environments: map[string]Environment{
			...
			"prod":  {URL: "https://api.quantflow.io", APIKey: "prod_key"},
		},
	}
	...
	if got.URL != "https://api.quantflow.io" || got.APIKey != "prod_key" {
...
func TestResolveActive_GlobalOverride(t *testing.T) {
	...
	if got.URL != "https://api.quantflow.io" {
...
func TestResolveActive_LegacyAuthAndEnvVar(t *testing.T) {
	...
	os.Setenv("QUANTFLOW_API_URL", "http://legacy.example.com")
