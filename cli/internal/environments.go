...
// SetConfigDir overrides the config directory (default: ~/.quantflow). Used by tests
...
// ConfigDir returns the resolved config directory (override if set, else ~/.quantflow).
func ConfigDir() (string, error) {
	...
	return filepath.Join(home, ".quantflow"), nil
}
...
// flag) > persisted active env > legacy auth.json + QUANTFLOW_API_URL > default.
func ResolveActive(override string) (*Environment, error) {
	...
	if name != "" {
		...
		if !ok {
			return nil, fmt.Errorf("environment %q is not defined (use `quantflow env add %s`)", name, name)
		}
		return env, nil
	}
	...
	if envs.Active != "" {
		...
		if !ok {
			return nil, fmt.Errorf("active environment %q is not defined; pick one with `quantflow env use <name>` or remove the stale entry from %s",
				envs.Active, envFileName)
		}
		return env, nil
	}

	// Legacy fallback: auth.json + QUANTFLOW_API_URL.
...
func legacyURL() string {
	if url := os.Getenv("QUANTFLOW_API_URL"); url != "" {
		return url
	}
...
// auth.json + QUANTFLOW_API_URL/default URL, stored under the name "default" and
...
func MigrateLegacyAuth() (*Environments, error) {
	...
	if auth != nil || os.Getenv("QUANTFLOW_API_URL") != "" {
