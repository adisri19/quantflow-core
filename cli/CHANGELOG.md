...
All notable changes to the quantflow CLI will be documented in this file.
...
### Removed
- **Self-update command** - Removed `quantflow self-update` command (use package manager or manual download)
- **Check-update command** - Removed `quantflow check-update` command
- **Startup update check** - Removed automatic update check on CLI startup
- **Backtest commands** - Removed `quantflow backtest deploy`, `quantflow backtest list`, and `quantflow backtest delete` commands as part of platform simplification
- **Backtest schema support** - The `quantflow custom-bot schema` command no longer accepts `backtest` as an entry point type; only `bot` schemas are supported
...
- File exclusion system with `.quantflowignore` support
