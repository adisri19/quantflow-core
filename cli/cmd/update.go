...
func NewUpdateCmd(currentVersion string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "update",
		Short: "Update quantflow CLI to the latest version",
		Long:  "Check for and install the latest version of quantflow CLI from GitHub Releases",
...
func runUpdate(currentVersion string, checkOnly bool, yes bool) {
	...
	if checkOnly {
		logger.Print("  Run 'quantflow update' to install")
		return
	}
