...
func NewBotCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "Deploy, list, update, and delete your trading bots on quantflow platform",
	}
...
func NewBotDeployCmd() *cobra.Command {
	return &cobra.Command{
		...
		Long:  "Deploy a new trading bot to quantflow platform",
...
func listBotInstances(cmd *cobra.Command, args []string) {
	...
	if len(bots) == 0 {
		...
		logger.Print("Deploy your first bot with 'quantflow bot deploy <config.json>'")
		return
	}
...
func deleteBotInstance(cmd *cobra.Command, args []string, skipConfirm bool) {
	...
	if deleteResp != nil && deleteResp.OrphanedVersion != nil {
		...
		logger.Print("  Run 'quantflow custom-bot delete %s --version %s' to clean up the code bundle.", ov.Name, ov.Version)
	}
}
