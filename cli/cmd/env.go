...
func NewEnvCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Manage named environments for the quantflow CLI.

Each environment stores its own API URL and API key. Switch between them
instantly with 'quantflow env use <name>' or override per-command with --env.

Examples:
  quantflow env add local --url http://localhost:3000
  quantflow env add prod  --url https://api.quantflow.io --api-key quantflow_xxx
	...
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if len(envs.Environments) == 0 {
				logger.Print("No environments defined. Run `quantflow env add <name> --url <url>` to create one.")
				return
			}
...
func newEnvRemoveCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if wasActive {
				logger.Info("No active environment; run `quantflow env use <name>` to pick one.")
			}
		},
	}
}
