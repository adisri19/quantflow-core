...
func TestCreateBotZipWithIgnoreFile(t *testing.T) {
	// Test that .quantflowignore file works correctly during ZIP creation
...
# Keep vendor and node_modules (they are protected anyway)`
	...
	ignoreFile := filepath.Join(tempDir, ".quantflowignore")
	if err := os.WriteFile(ignoreFile, []byte(ignoreContent), 0644); err != nil {
		t.Fatalf("Failed to create .quantflowignore: %v", err)
	}
...
func TestCreateBotZipWithComplexIgnorePatterns(t *testing.T) {
	...
	// Create complex .quantflowignore file
...
func TestCreateBotZipIgnoreFileIncluded(t *testing.T) {
	// Test that .quantflowignore file itself is NOT included in the ZIP
	...
	ignoreContent := `*.log
test/`
	if err := os.WriteFile(filepath.Join(tempDir, ".quantflowignore"), []byte(ignoreContent), 0644); err != nil {
		t.Fatalf("Failed to create .quantflowignore: %v", err)
	}
	...
	// .quantflowignore should NOT be in the ZIP (it's a hidden file)
	if zipFiles[".quantflowignore"] {
		t.Error(".quantflowignore file should not be included in ZIP")
	}
