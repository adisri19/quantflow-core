...
- Uses `quantflow.parse()` instead of function signature for config
- Uses `quantflow.metric()` instead of structlog with _metric field
- Uses Python's logging module for structured logging
- Uses `quantflow.success()` for result output
...
from quantflow import parse, success, error, metric, state
