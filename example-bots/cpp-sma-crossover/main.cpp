...
int main() {
    // Get configuration using quantflow SDK
    ...
    quantflow::state::set("last_run", {
        {"timestamp", quantflow::current_timestamp()}
    });

    // Load persistent state from previous runs
    auto persisted = quantflow::state::get("bot_state");
    BotState state;
    ...
    quantflow::log("Bot " + botId + " started - " + symbol + " SMA(" +
              std::to_string(shortPeriod) + "/" + std::to_string(longPeriod) +
              ") - loaded " + std::to_string(state.signalCount) + " signals");
    quantflow::log("v1.1.0 - update interval: " + std::to_string(updateIntervalMs) + "ms");
    ...
    while (true) {
        try {
            ...
            if (prices.size() < static_cast<size_t>(longPeriod)) {
                quantflow::log("Insufficient data for " + symbol + ": need " + std::to_string(longPeriod) + " prices, have " + std::to_string(prices.size()));
