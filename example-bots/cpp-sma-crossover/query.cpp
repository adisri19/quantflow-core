 ...
 *   - CLI: quantflow bot query <bot_id> /status
...
int main() {
    ...
    quantflow::query::handler("/status", [](const quantflow::query::Request& req) {
        auto state = quantflow::state::get("bot_state");
     ...
     */
    quantflow::query::handler("/signals", [](const quantflow::query::Request& req) {
        int limit = 10;
        try {
     ...
     */
    quantflow::query::handler("/sma-values", [](const quantflow::query::Request& req) {
        auto state = quantflow::state::get("bot_state");
