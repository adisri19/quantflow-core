...
This directory contains example bots demonstrating how to build custom trading bots with React dashboards using the `@quantflow/react` SDK.
...
The frontend uses the `@quantflow/react` SDK to access bot events:
...
The `useQuantFlowEvents()` hook provides:
...
The TypeScript examples use `@quantflow-io/quantflow-node` from GitHub Packages:

```json
{
  "dependencies": {
    "@quantflow-io/quantflow-node": "^0.3.1"
  }
}
```
...
```
@quantflow-io:registry=https://npm.pkg.github.com
